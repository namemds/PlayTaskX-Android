package com.cdut.playtask.ui.count;
...
/**
 */
public class WeekCountViewFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_week_count_view, container, false);
    ...
    public void drawLineChart(LineChart lineChart) {
        ...
        // 查询一周的数据
        ArrayList<CountItem> countItems = MainActivity.mDBMaster.mCountDAO.queryDataByWeek(new Timestamp(System.currentTimeMillis()));
        ...
        if (lineChart.equals(incomeChart)) {
            for (CountItem countItem : countItems) {
                if (countItem.getScore() > 0) {
                    int dayOfWeek = getDayOfWeek(countItem.getTime());
                    income[dayOfWeek - 1] += countItem.getScore();
        ...
        // 设置X轴标签为日期
        String[] daysOfWeek = new String[]{"", "周一", "周二", "周三", "周四", "周五", "周六", "周日"};
        ...
        int today = getDayOfWeek(new Timestamp(System.currentTimeMillis()));
        ...
        if (lineChart.equals(incomeChart)) {
            textView_income.setText("本周收入：" + total);
        } else if (lineChart.equals(expendChart)) {
            textView_expend.setText("本周支出：" + total);
        } else if (lineChart.equals(balanceChart)) {
            textView_balance.setText("本周结余：" + total);
        }
